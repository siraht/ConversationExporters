...
- Commit `0867794` in `/home/travis/Projects/ChatGPTExport` adds the minimal MV3 build/package/dashboard/runtime shape adapted from GrokExporter, with source-level provenance comments and ChatGPT-only `https://chatgpt.com/*` host permission.
...
- Commit `89ba629` in `/home/travis/Projects/ChatGPTExport` defines versioned archive, inventory-page, capture-journal, normalized graph, asset, workspace, account-artifact, and manifest types plus runtime validators for current ChatGPT listing/detail/account/session envelopes and reusable synthetic graph fixtures.
...
- Commit `fe80151` in `/home/travis/Projects/ChatGPTExport` completes the typed JSON endpoint set needed by later phases: main/archived listings, project index and project conversations, shared listings/details, batch and single details, account artifacts, and authenticated file-download descriptors. Exact identifier/cursor/page/batch constraints construct every method, path, query, and body inside the page bridge.
...
- Commit `bf31cb2` in `/home/travis/Projects/ChatGPTExport` adapts the proven Grok archive filesystem, safe-path, stable-JSON, byte-count, and SHA-256 primitives with file-level provenance; commit `6464494` implements ChatGPT inventory across main, archived, project index, every project cursor chain, and shared history.
...
- Commit `271cf72` in `/home/travis/Projects/ChatGPTExport` completes CG-C by changing explicit selection from one workspace to one-or-more workspaces, preflighting every selection, and creating a separate `ChatGPTExport-<workspace-fingerprint>/` archive below the chosen parent directory.
...
- Commit `370c5e5` in `/home/travis/Projects/ChatGPTExport` implements conservative batches of at most ten, exact requested/returned/missing ID evidence, duplicate detection, strict envelope parsing, graph-neighbor/current-node/cycle checks, and single-detail recovery for missing, duplicate, malformed, or suspicious batch records.
...
- Commit `7666dba` in `/home/travis/Projects/ChatGPTExport` adds atomic run journals, validated state transitions, content-addressed listing/detail/batch revisions, raw-completion markers, and resume checks that verify identity, listing sets, referenced paths, and byte hashes rather than trusting file presence.
...
- Commits `7c8e6e1` and `8336cc3` in `/home/travis/Projects/ChatGPTExport` add conservative request delay/concurrency control with cooperative pause/resume/cancel before the next request, plus complete archive enumeration for independent verification. Commit `c4247cb` audits inventory/completion/normalized sets, marker and byte hashes, raw-to-normalized graph traceability, content-addressed assets, temporary-file absence, byte totals, and writes validation reports, `archive.json`, and streaming import indexes.
...
- Created and verified public repository `https://github.com/siraht/ChatGPTExporter`; `main` and `origin/main` matched commit `bbb8b4fe266da5726ca6c9d88953e9412697eaea` before the plan-only follow-up. No live account or private archive data was accessed or published.
...
### 2026-08-02 — ChatGPT private full-account acceptance mirrored
...
- The required immediate repeat performed zero fetches and rebuilds and retained all 938 conversations unchanged. Live drift fixes shipped in ChatGPTExporter `0.1.6`; TypeScript, 83/83 tests across 19 files, privacy scanning over 72 tracked/unignored files, the production build, and packaged Chromium acceptance pass.
- Authenticated category sampling compared private DOM title/text anchors in memory and emitted aggregates only. All 14 present categories passed: calibration, short, long, old, new, branched, project, shared, cited/browsed, tool/code, uploaded-file, generated-image, Canvas, and deep-research. Archived membership was absent from the verified inventory and remains explicitly unavailable.
- Local and Flywheel unified-archive imports each created 938 versions on first import and zero on exact repeat. Flywheel acknowledged repeated selective pushes without retransmission; CASS reports healthy, ready, fresh, complete coverage, zero quarantine, and zero warnings. Sanitized probes returned only `chatgpt-web` results with logical-conversation and raw provenance.
- ChatGPTExporter commit `c5618b3` is tagged and pushed as `v0.1.6`. Its public history contains only aggregate acceptance evidence and excludes private archive paths, account/conversation identifiers, titles, bodies, asset names, credentials, signed URLs, and browser state.

### 2026-08-02 — Grok baseline reverified for consolidation

- The unchanged Grok source passes TypeScript, 30/30 tests across eight files, privacy scanning over 58 tracked/unignored files, production build, packaged Chromium bridge acceptance, and two byte-identical release packages with SHA-256 `c4957b3dfb679b651c7db2dc32eb5f26527b1829f5af30b2d74afdf039864e7a`.
- The private archive still has identical 946-member inventory, directory, index, validation, and completion-marker sets; zero normalized-hash mismatches; zero invalid conversation validations; zero missing raw source paths; 107/107 complete asset records with matching byte counts and hashes; and zero reported failures.
- This revision is the aggregate-only accepted Grok `v0.1.0` tag. With ChatGPT `v0.1.6` already accepted, Phase CG-G may now create `/home/travis/Projects/ConversationExporters` and extract common behavior under compatibility evidence without mutating either tagged repository.
...
- Added root CI, privacy enforcement, provenance, upgrade instructions, deterministic release verification, and the authoritative consolidation checklist. A detached clean worktree then passed fresh dependency installation, all 126 tests, both packaged Chromium extensions, privacy/build gates, and reproducible releases. The public `siraht/ConversationExporters` repository and main ref were confirmed; the final release is tagged `v0.1.0`.
