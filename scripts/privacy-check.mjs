...
const forbiddenPaths = /^(captures|exports|private|browser-profile|research\/repos)\//;
const forbiddenExtensions = /\.(?:har|crx|p12|pem|key)$/i;
const patterns = [
  { label: "authorization credential", regex: /\b(?:authorization|proxy-authorization)\s*[:=]\s*["']?(?:Bearer|Basic)\s+(?!\[REDACTED\])[A-Za-z0-9._~+/=-]{12,}/gi },
  { label: "Grok session cookie", regex: /\b(?:sso|sso-rw|auth_token)=(?!\[REDACTED\])[A-Za-z0-9%._~+/=-]{12,}/gi },
  { label: "signed URL credential", regex: /[?&](?:x-amz-signature|signature|sig|token)=(?!%?5B?REDACTED)[A-Za-z0-9%._~+/=-]{12,}/gi },
  { label: "private key", regex: /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/g },
  { label: "common API token", regex: /\b(?:ghp_[A-Za-z0-9]{30,}|github_pat_[A-Za-z0-9_]{40,}|sk-[A-Za-z0-9_-]{24,}|xai-[A-Za-z0-9_-]{24,})\b/g },
];
const findings = [];
