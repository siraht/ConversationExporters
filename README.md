...
After installing or reloading the extension, refresh any signed-in provider tabs so their page bridges are current. Click the extension icon to open the unified dashboard, then use **Sync now** beside any provider or **Sync all providers**. ChatGPT and Grok run their complete resumable capture and validation engines directly from this page; Claude, Gemini, and AI Studio run their incremental web syncs here as well. AI Studio must initialize both current RPC shapes after the extension loads, so refresh its saved prompt/history view and open any one saved prompt before the first sync.
