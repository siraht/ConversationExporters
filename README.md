...
- Claude captures every organization and conversation, project metadata, project instructions and documents, complete raw message trees, uploaded files and generated artifacts, resumable markers, and archive validation through Claude's authenticated web API.
- Gemini captures the complete paginated chat inventory, raw conversation branches and provider payloads, Gems, account metadata, uploads and generated Google-hosted media, resumable markers, and archive validation through Gemini's authenticated page RPCs.
- Google AI Studio captures the authenticated `ListPrompts` and current `ResolveDriveResource` requests made by the page, paginates every saved prompt, stores the complete raw prompt/settings/turn payloads, downloads referenced Drive and Google-hosted assets, and validates the resulting archive.
...
After installing or reloading the extension, refresh any signed-in provider tabs so their page bridges are current. Click the extension icon to open the unified dashboard, then use **Sync now** beside any provider or **Sync all providers**. Every provider runs a complete, resumable, validating export engine from this page; the provider-specific rows explain whether that means workspaces, projects, Gems, saved prompts, files, generated media, or other account objects. AI Studio must initialize both current RPC shapes after the extension loads, so refresh its saved prompt/history view and open any one saved prompt before the first sync.
...
2. Run the build and package commands above. Upload `packages/unified-extension/dist/releases/conversation-archive-chrome-0.3.0.zip` as a new item in the [Developer Dashboard](https://chrome.google.com/webstore/devconsole). The ZIP has `manifest.json` at its root and includes 16, 32, 48, and 128 pixel PNG icons.
